...
- [ ] Replace `app/globals.css` with full design system:
...
- [ ] Create `components/ui/Button.tsx` — primary / ghost / danger variants
- [ ] Create `components/ui/Card.tsx` — glass card wrapper
- [ ] Create `components/ui/Badge.tsx` — colored tag/badge component
- [ ] Create `components/ui/Chip.tsx` — skill/tech chip
- [ ] Create `components/ui/Input.tsx` — styled form input
- [ ] Create `components/ui/Modal.tsx` — overlay modal
- [ ] Create `components/ui/Avatar.tsx` — user avatar with fallback initials
- [ ] Create `components/ui/ScoreBar.tsx` — animated horizontal progress bar
- [ ] Create `components/ui/PageLayout.tsx` — page wrapper with sidebar slot

**Done when**: `localhost:3000` shows a polished dark navbar with Clerk sign-in/sign-up buttons.

---

## Phase 2 — Database Setup (Neon + Drizzle ORM)
> **Goal**: Full database schema defined, migrations run, DB client ready.

- [ ] Add `DATABASE_URL` to `.env.local` (Neon connection string)
- [ ] Install: `drizzle-orm`, `drizzle-kit`, `@neondatabase/serverless`, `dotenv`
- [ ] Create `lib/db/index.ts` — Neon + Drizzle client
- [ ] Create `lib/db/schema.ts` — all tables:
...
- [ ] Create `lib/db/queries/` typed query helpers:
...
**Done when**: `npx drizzle-kit studio` shows all 7 tables with correct schema.
...
| 1 | Design System + UI Components | ⬜ Next |
